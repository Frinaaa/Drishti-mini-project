...
// --- UPDATED INTERFACE to reflect new statuses ---
interface Report {
  _id: string;
  person_name: string;
  age: number;
...
// --- Status Badge Component (updated colors) ---
const StatusBadge = ({ status }: { status: Report['status'] }) => {
  const statusStyles: Record<Report['status'], { bg: string; color: string }> = {
    'Found': { bg: '#28a745', color: '#fff' },
    'Pending Police Verification': { bg: '#007bff', color: '#fff' }, // Now blue
    'Pending NGO Verification': { bg: '#ffc107', color: '#333' }, // Stays yellow
...
export default function PoliceAlertsScreen() {
  ...
  const fetchData = useCallback(async () => {
    // This function is now correct because the backend /api/reports is filtered
    try {
      ...
      Alert.alert('Error', `Could not load alerts: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  if (loading) {
    return (
      <View style={styles.centered}><ActivityIndicator size="large" color="#850a0a" /><Text style={styles.loadingText}>Fetching Alerts...</Text></View>
    );
  }

  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen options={{ title: 'Incoming Police Alerts' }} />
      <FlatList
        data={reports}
        ...
        ListEmptyComponent={
          <View style={styles.centered}>
            ...
            <Text style={styles.emptyText}>No alerts requiring action.</Text>
          ...
          <SafeAreaView style={styles.modalContainer}>
            <ScrollView>
              ...
              <View style={styles.detailsContainer}>
                <View style={{alignItems: 'flex-start', marginBottom: 20}}><StatusBadge status={selectedReport.status} /></View>
                <InfoRow label="Age" value={selectedReport.age} />
                <InfoRow label="Gender" value={selectedReport.gender} />
                <InfoRow label="Area PIN Code" value={selectedReport.pinCode} />
                <InfoRow label="Last Seen Details" value={selectedReport.last_seen} />
                <InfoRow label="Description / Identifying Marks" value={selectedReport.description || 'N/A'} />
                <InfoRow label="Report Filed On" value={new Date(selectedReport.reported_at).toLocaleString()} />
                <InfoRow label="Forwarded By (NGO)" value={selectedReport.user.name} />
              </View>

              {/* --- MODIFICATION: Updated conditional rendering and buttons --- */}
              {selectedReport.status === 'Pending Police Verification' && (
                <View style={styles.modalActionsContainer}>
                  <TouchableOpacity 
                    style={[styles.actionButton, styles.foundButton]}
                    onPress={() => handleUpdateStatus(selectedReport._id, 'found')}
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? <ActivityIndicator color="#fff" /> : <Ionicons name="checkmark-done-circle-outline" size={22} color="white" />}
                    <Text style={styles.actionButtonText}>Mark as Found</Text>
                  </TouchableOpacity>

                  <TouchableOpacity 
                    style={[styles.actionButton, styles.rejectButton]}
                    onPress={() => handleUpdateStatus(selectedReport._id, 'reject')}
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? <ActivityIndicator color="#fff" /> : <Ionicons name="close-circle-outline" size={22} color="white" />}
...
const styles = StyleSheet.create({
  ...
  loadingText: { marginTop: 10, color: '#5B4242' },
  emptyText: { marginTop: 15, fontSize: 18, color: '#A47171', fontWeight: '600' },
  emptySubtext: { marginTop: 5, fontSize: 14, color: '#A47171' },
  ...
  cardImage: { width: 70, height: 70, borderRadius: 35, marginRight: 15, backgroundColor: '#f0f0f0' },
  cardContent: { flex: 1 },
  ...
  modalTitle: { fontSize: 24, fontWeight: 'bold', color: '#3A0000', flex: 1, marginRight: 10 },
  ...
  infoValue: { fontSize: 16, color: '#3A0000', lineHeight: 22 },
  modalActionsContainer: { paddingHorizontal: 20, paddingVertical: 15, borderTopWidth: 1, borderColor: '#F0E0E0', backgroundColor: '#FFFFFF' },
  actionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 10, paddingVertical: 14, marginBottom: 10 },
  actionButtonText: { color: 'white', fontSize: 16, fontWeight: 'bold', marginLeft: 10 },
  foundButton: { backgroundColor: '#28a745' }, // Changed from verifyButton
